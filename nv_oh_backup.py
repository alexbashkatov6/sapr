...
from config import FILE_NAME_TO_CLASSES, MAIN_CLASSES_TREE
...
class VCInSet(ValueChecker):
    pass


class VCStaticInSet(VCInSet):
    pass


class VCDynamicInSet(VCInSet):
    pass
...
def cyclic_find(complex_attr: ComplexAttribute, attr_address: AttributeAddress, find_single_attribute: bool = False) -> \
    ...
    for i, attr_index in enumerate(ail):
        ...
        if cycle_complex_attr.single_attribute_type is str:
            assert len(ail) - 1 == i
        else:
...
class NewDefaultDescriptor:
    ...
    def __init__(self,
                 ...
                 min_count: int = 0,
                 single_attribute_type: Any = str,
                 value_checker: ValueChecker = DEFAULT_VALUE_CHECKER,
                 value_suggester: Suggester = DEFAULT_VALUE_SUGGESTER):
        self.complex_attribute_template = ComplexAttribute(is_list=is_list,
    ...
    def __set__(self, instance, command: ComplexAttributeManagementCommand):
        ...
        if command_ == AttributeCommand.set_single:
            ...
            cycle_complex_attr, single_attrib = cyclic_find(complex_attr, attr_address, True)
            single_attrib.last_input_value = value
            return
        else:
            cycle_complex_attr, _ = cyclic_find(complex_attr, attr_address)
...
class ObjectsHandler(QObject):
    ...
    def __init__(self):
        ...
        self.objects_tree: OrderedDict[str, OrderedDict[str, PpoObject]] = PPO_OBJECTS  # output structure
    ...
    def got_object_name(self, name: str):
        ...
        if name in self.obj_name_to_cls_name_dict:
            ...
            # 1. rollback - last accepted - handling
    ...
    def new_exchange_attr_format(self, obj: PpoObject):
        pass
